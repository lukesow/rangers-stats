...
def generate_random_bg_svg():
    ...
    for _ in range(20):
        ...
        # Random Opacity (Very subtle ghost effect)
        opacity = random.uniform(0.02, 0.06)
        
        # Transform: Rotate around center of the lion (approx 50,50 for this path) then translate
        transform = f"translate({x}, {y}) rotate({rot} 50 50) scale({scale})"
        
        svg_elements.append(
            f"<path d='{lion_path_d}' fill='#add8e6' opacity='{opacity}' transform='{transform}' />"
        )
...
lion_b64 = generate_random_bg_svg()
...
st.markdown(f"""
    ...
    /* SIDEBAR WITH RANDOM LION PATTERN */
...
def load_data():
    try:
        ...
        # Ensure Day/Month/Year are numeric, coercing errors to 0/NaN
        for c in ['Day', 'Month', 'Year']:
            if c in df.columns:
                df[c] = pd.to_numeric(df[c], errors='coerce').fillna(0).astype(int)
        
        # Construct date string only if columns exist
...
if st.session_state['page'] == 'single':
    if not players_list:
        st.info("👋 Welcome! The database is empty or CSV format is incorrect. Please check the **Admin Panel** for load errors.")
    else:
        # --- CONTROL BAR ---
