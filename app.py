    ...
    if not players_list:
        ...
        if not p_df.empty:
            ...
            with tab3:
                ...
                if starts > 0:
                    # Get all games where current player started
                    start_games = p_df[p_df['Role'] == 'Starter']
                    
                    # Extract all teammates in those games
                    teammate_data = []
                    
                    # Iterate through the games where our player started
                    for idx, row in start_games.iterrows():
                        result = row['ResultCode']
                        # Get all players in R1-R11 for this specific game
                        team_in_match = row[[f'R{i}' for i in range(1, 12)]].values
                        
                        for mate in team_in_match:
                            if mate and str(mate) != 'nan' and mate != sel_p:
                                teammate_data.append({'Teammate': mate, 'Result': result})
                    
                    if teammate_data:
                        tm_df = pd.DataFrame(teammate_data)
                        
                        # Group by Teammate to get Total Games and Win Counts
                        tm_stats = tm_df.groupby('Teammate').agg(
                            Apps=('Result', 'count'),
                            Wins=('Result', lambda x: (x == 'W').sum())
                        ).reset_index()
                        
                        # Calculate Win Rate
                        tm_stats['Chemistry'] = (tm_stats['Wins'] / tm_stats['Apps'])
                        
                        # Sort by Apps then Win Rate
                        tm_stats = tm_stats.sort_values(['Apps', 'Chemistry'], ascending=[False, False])
                        ...
                        st.info("No teammates found in starting lineups.")
...
# --- HEAD TO HEAD ---
    ...
    if len(players_list) < 2:
        ...
        if p1 == p2: st.error("Select different players.")
            ...
            # Helper for H2H Individual Stats
            def get_h2h_stats(p):
            ...
            # Helper for Chemistry (Both started together)
            def get_partnership_chem(pA, pB):
                # Find games where BOTH are in Starters (R1-R11)
            ...
            if chem_games > 0:
                st.progress(chem_rate / 100)
            else:
                st.caption("These players have never started a match together in the current selection.")
